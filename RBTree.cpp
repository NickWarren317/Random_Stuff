...
template <typename keytype, typename valuetype>
    class RBNode{
        public:
            RBNode<keytype, valuetype> *left,*right,*parent;
            keytype key;
            valuetype val;
            bool red;
            int siz;
...
class RBTree{
  private:
    ...
    void postOrderDelete(RBNode<keytype, valuetype> *x){ //print postorder transversal
        if(x == nil){
            return;
        }
...
/*int main(){
	...
	for (int i=0;i<size;i+=removeIndex){
         X.remove(i); 
    }
	cout << "done deleting" << endl;
    X.printHelper(X.root,"", true);
}
/*
    FIX:
        Copy Constructor
